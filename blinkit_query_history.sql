...
SELECT
    ...
    roas AS stored_roas,
    ROUND(revenue_generated / spend, 2) AS calculated_roas
FROM marketing_performance
ORDER BY ABS(roas - (revenue_generated / spend)) DESC
LIMIT 10;
SELECT
    campaign_name,
    channel,
...
SELECT COUNT(DISTINCT delivery_partner_id) AS unique_partners, COUNT(*) AS total_deliveries
FROM delivery_performance;
SELECT
    delivery_partner_id,
    COUNT(*)                                                          AS total_deliveries,
...
LIMIT 10;
SELECT COUNT(DISTINCT store_id) AS unique_stores, COUNT(*) AS total_orders
FROM orders;
SELECT
    store_id,
